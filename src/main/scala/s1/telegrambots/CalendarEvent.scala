...
class CalendarEvent (var startTime:String,var endTime:String):
  ...
  def startHour:Int=if startTime.length>10 then startTime.substring(9,11).toInt+3 else 0
  ...
  def endHour:Int=if endTime.length>10 then endTime.substring(9,11).toInt+3 else 0
  ...
  /** Returns true if two events "overlap" ie. exist at the same time even for a moment but are not "covered"*/
